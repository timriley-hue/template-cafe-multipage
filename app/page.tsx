    ...
    <>
      ...
      <section className="bg-[var(--color-accent)] py-24 px-4 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <Link
            href={hero.cta.href}
            className="inline-block bg-[var(--color-brand)] text-[var(--color-brand-text)] px-8 py-3 rounded-full font-medium hover:bg-[var(--color-brand-dark)] transition-colors"
          >
            {hero.cta.label}
          </Link>
        </div>
      ...
      <section className="bg-[var(--color-brand)] text-[var(--color-brand-text)] py-8 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          {hours.notes && (
            <p className="text-center text-xs opacity-50 mt-3">{hours.notes}</p>
          )}
        </div>
      </section>
      ...
      <section className="py-20 px-4 text-center bg-white">
        <div className="max-w-xl mx-auto">
          <h2
            className="text-3xl font-bold mb-4 text-[var(--color-brand-dark)]"
            style={{ fontFamily: "var(--font-heading)" }}
          >
            What we serve
          </h2>
          ...
          <Link
            href="/menu"
            className="inline-block border-2 border-[var(--color-brand)] text-[var(--color-brand)] px-8 py-3 rounded-full font-medium hover:bg-[var(--color-brand)] hover:text-[var(--color-brand-text)] transition-colors"
          >
            View full menu
          </Link>
        </div>
