...
export default function SiteHeader() {
  ...
  return (
    <header
      className={`sticky top-0 z-50 bg-[var(--color-brand)] text-[var(--color-brand-text)] transition-shadow duration-200 ${
      ...
      <div className="max-w-5xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-7 text-sm font-medium">
          ...
          {modules.orderOnline && (
            <a
              href="#order"
              className="ml-2 bg-white/15 hover:bg-white/25 px-4 py-1.5 rounded-full text-sm font-medium transition-colors"
            >
              Order online
            </a>
          )}
